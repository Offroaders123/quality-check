...
# This one is specifically for new imports from a list of WAVs, to M4A.
# Eventually, I will (and should in general now, for maintainability) make these
# variants into a single script. There are only small changes to each one at this point.
# Maybe I could convert it to TS too. Or is Bash-only more logical? I just don't like that
# it's not 'type-safe'. I get worried about writing code blindly now, especially
# for something crucial like this script.
...
LOSSLESS_DIR="$1"   # Directory containing lossless source files (e.g., WAV)
...
for lossless_file in "$LOSSLESS_DIR"/*.wav; do
    ...
    # copies all global metadata from in.aiff to out.m4a, and
    # copies audio stream metadata from in.aiff to out.m4a
